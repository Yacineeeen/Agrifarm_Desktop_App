...
public class FrontboardController implements Initializable {
    ...
    @FXML private Button produitConsulterBtn;
    ...
    private void configureMenusByRole(Role role) {
       ...
       // if (animauxMesAnimauxBtn != null) animauxMesAnimauxBtn.setManaged(isAgriculteur);
        if (animauxMesAnimauxBtn != null) {
            animauxMesAnimauxBtn.setVisible(true);
            animauxMesAnimauxBtn.setManaged(true);
        }
        ...
        if (produitConsulterBtn != null) produitConsulterBtn.setVisible(true);
    ...
    private void handleEtudeConsulter() {
        loadContent("/AfficherEtudeF.fxml");
    }
    @FXML
    private void handleCultureConsulter() {
        loadContent("/AfficherCultureF.fxml");
    }
    @FXML
    private void handleAnimauxMesAnimaux() {
        loadContent("/AnimalReadView.fxml");
    }

    @FXML
    private void handleProduitConsulter() {
        loadContent("/AfficherCommande.fxml");
    }
    @FXML
    private void handleAnimauxConsulterVeto() {
        loadContent("/VeterinaireReadView.fxml");
    }

    @FXML
    private void handleAbonnementMon() {
        loadContent("/AfficherAbonnement.fxml");
    }

    @FXML
    private void handleAbonnementFactures() {
        loadContent("/AfficherFacture.fxml");
    }
    ...
    private void handleMachineConsulter() {
        try {
            // Get resource using class loader
            URL url = getClass().getClassLoader().getResource("AfficherMachineF.fxml");
            if (url == null) {
                showError("Could not find FXML file");
                return;
            }
            ...
            e.printStackTrace();
            showError("Error loading machine view: " + e.getMessage());
        }
    }
    @FXML
