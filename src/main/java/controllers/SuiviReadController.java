...
public class SuiviReadController {
    ...
    public void initialize() {
        try {
            ...
            suiviService.addAnalysisColumnIfNotExists();
            ...
            showAlert(Alert.AlertType.ERROR, "Initialization Error", "Error initializing SuiviReadController: " + e.getMessage());
            logger.error("Error initializing SuiviReadController", e);
            e.printStackTrace();
        }
    }
            ...
            for (Suivi suivi : suivis) {
                if (suivi.getAnimal() != null) {
                    try {
                        ...
                        logger.error("Error loading animal for suivi: " + e.getMessage());
                    }
                }
                if (suivi.getVeterinaire() != null) {
                    try {
                        ...
                        logger.error("Error loading veterinaire for suivi: " + e.getMessage());
                    }
                }
            }
            suiviTableView.setItems(FXCollections.observableArrayList(suivis));
        } catch (Exception e) {
            showAlert(Alert.AlertType.ERROR, "Data Loading Error", "Error loading suivi data: " + e.getMessage());
            logger.error("Error loading suivi data", e);
            e.printStackTrace();
        }
    }
    ...
    private void handleAddSuivi() {
        try {
            URL location = getClass().getResource("/SuiviAddView.fxml");
            if (location == null) {
    ...
    private void openUpdateSuiviForm(Suivi suivi) {
        ...
        try {
            URL location = getClass().getResource("/SuiviUpdateView.fxml");
            if (location == null) {
    ...
    private void openDeleteSuviForm(Suivi suivi) {
        ...
        try {
            URL location = getClass().getResource("/SuiviDeleteView.fxml");
    ...
    private void handleOpenAnimaux() {
        try {
            URL location = getClass().getResource("/AnimalReadView.fxml");
            if (location == null) {
