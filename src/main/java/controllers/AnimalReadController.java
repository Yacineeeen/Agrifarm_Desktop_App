...
import services.GestionAnimal.IAnimalService;
import services.GestionAnimal.ISuiviService;
import services.GestionAnimal.VeterinaireService;
import services.GestionAnimal.AnimalServiceImpl;
...
public class AnimalReadController {
    ...
    private void handleAddAnimal() {
        try {
            // Utiliser la nouvelle méthode de chargement avec des chemins relatifs
            FXMLLoader loader = new FXMLLoader();
            loader.setLocation(getClass().getResource("/AnimalAddView.fxml"));

            // Vérifier si la ressource est trouvée
            if (loader.getLocation() == null) {
                // Essayer avec un autre chemin si le premier échoue
                loader.setLocation(getClass().getResource("AnimalAddView.fxml"));

                if (loader.getLocation() == null) {
                    throw new IOException("Impossible de trouver le fichier AnimalAddView.fxml");
                }
            }
            ...
            // Créer et configurer une nouvelle fenêtre
            ...
            // Recharger les données après la fermeture
            loadAnimalData();
        } catch (IOException e) {
            e.printStackTrace();
